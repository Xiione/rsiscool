...
GaloisFieldPolynomial::GaloisFieldPolynomial() { poly.clear(); }

GaloisFieldPolynomial::GaloisFieldPolynomial(const unsigned int size,
...
GaloisFieldPolynomial::GaloisFieldPolynomial(
    const GaloisFieldPolynomial &polynomial) {
...
bool GaloisFieldPolynomial::valid() const { return (poly.size() > 0); }
...
GaloisFieldPolynomial::operator/=(const GaloisFieldPolynomial &divisor) {
  if ((deg() >= divisor.deg()) && (divisor.deg() >= 0)) {
...
const GaloisFieldElement &
GaloisFieldPolynomial::operator[](const unsigned int &term) const {
  assert(term < poly.size());
  return poly[term];
}

GaloisFieldElement &
GaloisFieldPolynomial::operator[](const unsigned int &term) {
  assert(term < poly.size());
  return poly[term];
}

GaloisFieldElement
GaloisFieldPolynomial::operator()(const GaloisFieldElement &value) {
...
const GaloisFieldElement
GaloisFieldPolynomial::operator()(const GaloisFieldElement &value) const {
  GaloisFieldElement result(0);
...
const GaloisFieldElement
GaloisFieldPolynomial::operator()(GFSymbol value) const {
  return (*this)(GaloisFieldElement(value));
}

bool GaloisFieldPolynomial::operator==(
    const GaloisFieldPolynomial &polynomial) const {
  if (poly.size() != polynomial.poly.size())
    return false;
  else {
    for (unsigned int i = 0; i < poly.size(); i++) {
...
void GaloisFieldPolynomial::simplify() { simplify(*this); }
...
GaloisFieldPolynomial operator+(const GaloisFieldPolynomial &a,
                                const GaloisFieldPolynomial &b) {
...
GaloisFieldPolynomial operator+(const GaloisFieldElement &a,
                                const GaloisFieldPolynomial &b) {
...
GaloisFieldPolynomial operator+(const GFSymbol &a,
                                const GaloisFieldPolynomial &b) {
...
GaloisFieldPolynomial gcd(const GaloisFieldPolynomial &a,
                          const GaloisFieldPolynomial &b) {
  if ((!a.valid()) && (!b.valid()))
