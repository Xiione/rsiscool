...
GaloisField::GaloisField(const int pwr, const unsigned int *primitive_poly)
  ...
  for (unsigned int i = 0; i < power; i++) {
    prim_poly_hash += ((i & 1) == 0)
                          ? ((prim_poly_hash << 7) ^ primitive_poly[i] ^
                             (prim_poly_hash >> 3))
                          : (~((prim_poly_hash << 11) ^ primitive_poly[i] ^
                               (prim_poly_hash >> 5)));
  }
...
bool GaloisField::operator==(const GaloisField &gf) {
  return ((this->power == gf.power) &&
          (this->prim_poly_hash == gf.prim_poly_hash));
}

// added to silence c++20 standard warning
bool GaloisField::operator!=(const GaloisField &gf) { return !(*this == gf); }
